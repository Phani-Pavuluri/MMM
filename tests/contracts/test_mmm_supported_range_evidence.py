...
def _restriction(code: str = "unsupported_range") -> MMMRangeRestriction:
    return MMMRangeRestriction(
        ...
        technical_summary="Claims outside governed range remain blocked.",
        affected_scope=_scope(),
        evidence_references=["diagnostic:range-001"],
        affected_technical_claims=[
            MMMTechnicalClaim.EXTRAPOLATIVE_SIMULATION,
            MMMTechnicalClaim.BUDGET_OPTIMIZATION_INPUT,
        ],
        claim_effects=[
            MMMClaimEffect(
                claim=MMMTechnicalClaim.EXTRAPOLATIVE_SIMULATION,
                disposition=MMMTechnicalClaimDisposition.BLOCKED,
                blocking_references=["diagnostic:range-001"],
            )
        ],
    )
...
def _record(**overrides: object) -> MMMSupportedRangeRecord:
    data: dict[str, object] = {
        "range_record_id": "range-search-001",
        "run_id": "run-range-001",
        "model_id": "model-ridge-001",
        "model_family": "ridge",
        "configuration_hash": "sha256:config",
        "scope": _scope(),
        "observed_lower": _bound(100.0),
        "observed_upper": _bound(1000.0),
        "supported_lower": _bound(110.0),
        "supported_upper": _bound(980.0),
...
def _evidence(**overrides: object) -> MMMSupportedRangeEvidence:
    data: dict[str, object] = {
        "evidence_id": "range-evidence-001",
        "run_id": "run-range-001",
        "created_at": NOW,
...
def test_public_contract_is_versioned_deterministic_and_preserves_observed_supported_validated_distinction() -> None:
    evidence = _evidence(
        records=[
            _record(
                validated_lower=_bound(125),
                validated_upper=_bound(950),
                validation_evidence_references=["validation:holdout-001"],
            )
        ]
    )
    assert evidence.schema_version == MMM_SUPPORTED_RANGE_SCHEMA_VERSION
    assert evidence.summary_counts == {
        "available": 1,
...
def test_unavailable_blocked_research_only_and_pre_model_evidence_are_truthful() -> None:
    unavailable = _record(
        range_record_id="range-observed-only",
        model_id=None,
        observed_lower=_bound(100),
        observed_upper=_bound(1000),
        supported_lower=None,
        supported_upper=None,
        availability_status=MMMRangeAvailabilityStatus.UNAVAILABLE,
        range_relation=MMMRangeRelation.UNKNOWN,
        extrapolation_classification=MMMExtrapolationClassification.UNKNOWN,
    )
    blocked = _record(
        range_record_id="range-blocked",
        availability_status=MMMRangeAvailabilityStatus.BLOCKED,
        extrapolation_classification=MMMExtrapolationClassification.UNSUPPORTED_EXTRAPOLATION,
        range_relation=MMMRangeRelation.OUTSIDE_SUPPORTED_RANGE,
        restrictions=[_restriction()],
        limitation_references=["limitation:range-001"],
    )
    research = _record(
        range_record_id="range-bayes",
        model_family="bayesian",
        availability_status=MMMRangeAvailabilityStatus.RESEARCH_ONLY,
        evidence_basis=[MMMRangeEvidenceBasis.RESEARCH_ONLY],
        restrictions=[_restriction("research_only")],
        extrapolation_classification=MMMExtrapolationClassification.UNKNOWN,
        range_relation=MMMRangeRelation.UNKNOWN,
    )
    ...
    assert aggregate.summary_counts is not None
...
def test_extrapolation_and_uncertainty_rules_are_explicit() -> None:
    ...
    boundary = _record(
        range_relation=MMMRangeRelation.AT_UPPER_BOUNDARY,
        extrapolation_classification=MMMExtrapolationClassification.BOUNDARY,
        restrictions=[_restriction("upper_boundary")],
    )
    assert boundary.extrapolation_classification == MMMExtrapolationClassification.BOUNDARY


def test_simulation_eligibility_is_additive_fail_closed_and_availability_consistent() -> None:
    legacy = _record()
    assert legacy.simulation_eligibility == MMMSupportedRangeSimulationEligibility.NOT_ASSESSED
    assert (
        _record(simulation_eligibility=MMMSupportedRangeSimulationEligibility.NOT_ELIGIBLE).availability_status
    ...
    assert (
        ...
        == MMMSupportedRangeSimulationEligibility.ELIGIBLE_FOR_TECHNICAL_SIMULATION
    )
    for status in (
        MMMRangeAvailabilityStatus.PARTIALLY_AVAILABLE,
        MMMRangeAvailabilityStatus.UNAVAILABLE,
        MMMRangeAvailabilityStatus.BLOCKED,
        ...
        if status == MMMRangeAvailabilityStatus.PARTIALLY_AVAILABLE:
            values.update({"supported_lower": None, "supported_upper": None})
        if status == MMMRangeAvailabilityStatus.UNAVAILABLE:
            values.update({"supported_lower": None, "supported_upper": None})
        if status == MMMRangeAvailabilityStatus.BLOCKED:
            values.update({"limitation_references": ["limitation:range"]})
        if status == MMMRangeAvailabilityStatus.RESEARCH_ONLY:
            values.update({"evidence_basis": [MMMRangeEvidenceBasis.RESEARCH_ONLY]})
        with pytest.raises(ValidationError, match="eligibility requires AVAILABLE"):
            _record(**values)


def test_manifest_and_export_boundary_link_range_evidence_additively() -> None:
    context = MMMExportRuntimeContext(
        model_run_id="run-range-001",
        training_data_fingerprint="sha256:panel",
        model_artifact_fingerprint="sha256:config",
        generated_at="2026-07-15T12:00:00Z",
        package_version="0.1.0",
        git_commit="abc1234",
        model_form="ridge",
        estimand="modeled_revenue",
        time_window="2026-01-01/2026-06-30",
        geo_scope="national",
        channel_scope=("search",),
    ...
    result = adapt_runtime_artifacts_to_export_manifest_outcome(
        context=context,
        manifest_id="manifest-range-001",
        created_at=NOW,
...
@pytest.mark.parametrize(
    "fixture_name",
    [
        "available_ridge.json",
        "observed_only_unavailable.json",
        "restricted_boundary.json",
        "unsupported_extrapolation.json",
        "pre_model_insufficient_evidence.json",
        "bayesian_research_only.json",
        "multi_channel_distinct.json",
    ],
)
def test_deterministic_supported_range_fixtures_parse_and_round_trip(fixture_name: str) -> None:
    ...
    assert not any(
        ...
        for value in ("/users/", "traceback", "stack trace", "dataframe", "secret", "password")
    )
