...
def test_describe_research_normalization_includes_divisor_and_factors() -> None:
    raw = (0.2, 0.4, 0.05, 0.1, 0.3)
    norm = normalize_objective_vector(raw, NormalizationProfile.RESEARCH)
    ...
    assert rep["rule"].startswith("research_soft_normalize")
    assert "soft_normalize_divisor" in rep
    assert len(rep["per_component_factor_vs_divisor"]) == 5
