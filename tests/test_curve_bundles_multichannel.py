...
def _curve_bundle_record(
    ...
    m = [0.01 * (i + 1) for i in range(len(grid))]
    core = {
        "channel": ch,
        "spend_grid": grid,
        "response_on_modeling_scale": resp,
        "marginal_roi_modeling_scale": m,
    }
...
def test_extension_report_has_curve_bundles_per_channel():
    ...
    assert rep["decision_bundle"]["bundle_version"] == "mmm_decision_bundle_v2"
...
def test_cli_curve_bundle_file_runs(tmp_path, monkeypatch):
    monkeypatch.setenv("MMM_UNSAFE_LEGACY_DIAGNOSTIC_BUDGET", "1")
    ...
    curve_path.write_text(
        json.dumps({"curve_bundles": [_curve_bundle_record("c1", [1.0, 5.0, 10.0], [0.1, 0.4, 0.45], ec)]}),
        encoding="utf-8",
    )
    yaml = tmp_path / "c.yaml"
    yaml.write_text(
...
def test_cli_legacy_curve_optimizer_blocked_without_env_ack(tmp_path, monkeypatch):
    monkeypatch.delenv("MMM_UNSAFE_LEGACY_DIAGNOSTIC_BUDGET", raising=False)
    from mmm.cli import main as cli_main
...
""",
    ...
    assert "MMM_UNSAFE_LEGACY_DIAGNOSTIC_BUDGET" in out
                ...
                "curve_bundles": [
                    {
                        ...
                        "marginal_roi_modeling_scale": [0.01, 0.02, 0.03],
                        "typed_curve_quantity": parse_legacy_curve_bundle_dict(
                            {
                                "channel": "c1",
                                "spend_grid": [1.0, 5.0, 10.0],
                                "response_on_modeling_scale": [0.1, 0.4, 0.45],
...
""",
    ...
    assert (
        "contract_version" in err.lower()
        or "economics_contract" in err.lower()
        or "typed_curve_quantity" in err.lower()
    )
...
def test_cli_prod_optimize_budget_full_model_smoke(tmp_path):
    """Prod path: panel CSV + extension ridge_fit_summary → SLSQP on simulate() Δμ (``mmm decide optimize-budget``)."""
    from mmm.cli import main as cli_main
...
data:
  ...
  control_columns: []
  data_version_id: curve-bundle-prod-snapshot
budget:
  total_budget: 100
extensions:
  optimization_gates:
...
""",
    ...
    assert result.exit_code == 0, result.stdout + (result.stderr or "")
    written = out_bundle.read_text(encoding="utf-8")
    assert "full_model_simulation_slsqp" in written
    assert "recommended_spend_plan" in written
