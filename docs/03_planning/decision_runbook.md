...
- **`data_fingerprint`** and **`panel_fingerprint`** (must match). Prefer **`sha256_combined`** when present (fingerprint v2); older bundles may only expose **`sha256_panel_keycols_sorted_csv`** — see [planning_artifact_schema.md](planning_artifact_schema.md#data_fingerprint--panel_fingerprint).
- **`package_version`**
