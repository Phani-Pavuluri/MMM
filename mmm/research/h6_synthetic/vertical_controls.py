"""H6b — vertical-specific control profiles for production-shaped synthetic worlds."""
...
class VerticalControlProfile:
    """Recommended controls for a vertical with known generative effects."""
...
VERTICAL_RETAIL = VerticalControlProfile(
    ...
    notes="Promo and holiday are required; competitor/weather optional for stress variants.",
)
...
VERTICAL_CPG = VerticalControlProfile(
    ...
    notes="Distribution and promo depth often collinear with trade media in real panels.",
)
...
VERTICAL_AUTO = VerticalControlProfile(
    ...
    notes="Incentive and national TV often co-move — stress via media_correlated_controls.",
)
...
def get_vertical_profile(vertical_id: str) -> VerticalControlProfile:
    if vertical_id not in VERTICAL_PROFILES:
        raise KeyError(f"unknown H6 vertical: {vertical_id!r}; known: {sorted(VERTICAL_PROFILES)}")
...
def control_truth_for_profile(
    ...
    *,
    ...
    """Record control truth included in world artifact (H6b)."""
